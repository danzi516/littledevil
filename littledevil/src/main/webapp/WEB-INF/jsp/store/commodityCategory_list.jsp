...
<div class="console-container">
        ...
        <div class="col-sm-12">
            <div class="console-title clearfix">
                <div class="pull-left"><a href="javascript:" class="btn btn-default">商品分类管理</a> </div>
            </div>
            <hr/>
	 <div id="toolbar">
        <div class="pull-left"><a href="${webRoot}/store/toAddCommodityCategory" role="button" class="btn btn-primary add" target="mainFrame" >添加分类</a></div>&nbsp;&nbsp;&nbsp;&nbsp; 
        ...
        <div style="display: inline-table; width: 100px;">
        	<select class="form-control" id="state" onchange="refalshData()">
        		...
        		<option value="1">上线分类</option>
                <option value="0">下线分类</option>
        	</select>
        </div></div>
        <table id="table"
		       ...
		       data-ajax="getCommodityCategoryList"
    ...
    $table.bootstrapTable({
        columns: [
            [
             ...
             {field: 'categoryName',title: '分类名称',sortable: false, align: 'center'},
             {field: 'creatTime',title: '创建时间',sortable: false, align: 'center'},
             {field: 'updateTime',title: '更新时间',sortable: false, align: 'center'},
...
function operateFormatter(value, row, index) {
    ...
    html.push('<a class="delete btn" style="margin-left: 5px;font-size:1em" href="javascript:void(0)" target="mainFrame">删除</a>');
    html.push('<a class="detail btn" style="margin-left: 5px;font-size:1em" href="javascript:void(0)" target="mainFrame">详情(没做)</a>');
    return html.join('');
}
...
function refalshData(){
 ...
 function getCommodityCategoryList(params){
    	var url="${webRoot}/commodityCategory/selectByPage";
 ...
 function deleteUser(row){
 	if(confirm("您确定要删除分类:"+row.categoryName+" 吗?"))
		{
	    	var url="${webRoot}/commodityCategory/delete/"+row.id;
	    	var code="";
	    	$.ajax({
				type : "post",
				url : url,
 ...
 function detailUser(row){
	 alert(11111);
	 location.href = "${webRoot}/store/toCommodityCategorydetail/"+row.id;
 }
 function enableUser(row){
 	if(confirm("您确定要上线:"+row.categoryName+" 吗?"))
		{
	    	var url="${webRoot}/commodityCategory/update";
	    	var code="";
	    	var data={
	    		"id":row.id,
	    		"state":'1'
 ...
 function disableUser(row){
 	if(confirm("您确定要下线:"+row.categoryName+" 吗?"))
		{
	    	var url="${webRoot}/commodityCategory/update";
	    	var data={
	    		"id":row.id,
