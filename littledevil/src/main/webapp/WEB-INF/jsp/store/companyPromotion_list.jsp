...
<div class="console-container">
        ...
        <div class="col-sm-12">
            <div class="console-title clearfix">
                <div class="pull-left"><a href="javascript:" class="btn btn-default">商户活动管理</a> </div>
            </div>
            <hr/>
	 <div id="toolbar">
        <div class="pull-left"><a href="${webRoot}/companyPromotion/toAddCompanyPromotion" role="button" class="btn btn-primary add" target="mainFrame" >添加活动</a></div>&nbsp;&nbsp;&nbsp;&nbsp; 
        ...
        <div style="display: inline-table; width: 100px;">
        	<select class="form-control" id="state" onchange="refalshData()">
        		...
        		<option value="1">上线活动</option>
                <option value="0">下线活动</option>
        	</select>
        </div></div>
        <table id="table"
		       ...
		       data-ajax="getCompanyPromotionList"
    ...
    $table.bootstrapTable({
        columns: [
            [
             ...
             {field: 'promotionName',title: '活动名称',sortable: false, align: 'center'},
             {field: 'promotionPrice',title: '活动价格',sortable: false, align: 'center'},
             {field: 'promotionLogo',title: '活动图标',sortable: false, align: 'center'},
             {field: 'promotionNote',title: '活动简介',sortable: false, align: 'center'},
             {field: 'state',title: '活动状态',sortable: false, align: 'center'},
...
function refalshData(){
 ...
 function getCompanyPromotionList(params){
    	var url="${webRoot}/companyPromotion/selectByPage";
 ...
 function deleteUser(row){
 	if(confirm("您确定要删除商品:"+row.PromotionName+" 吗?"))
		{
 ...
 function enableUser(row){
 	if(confirm("您确定要上线:"+row.PromotionName+" 吗?"))
		{
	    	var url="${webRoot}/companyPromotion/update";
	    	var code="";
	    	var data={
	    		"id":row.id,
	    		"state":'1'
 ...
 function disableUser(row){
 	if(confirm("您确定要下线:"+row.PromotionName+" 吗?"))
		{
	    	var url="${webRoot}/companyPromotion/update";
	    	var data={
	    		"id":row.id,
